        ...
        namespace OSM {
            ...
            class PBF : public Base {
                    ...
                    if (pbf_nodes) {
                        ...
                        if (pbf_nodes->has_dense()) {
                            ...
                            for (int i=0, l=dense->keys_vals_size(); i<l; i++) {
                                ...
                                if (sid > 0) {
                                    dense->set_keys_vals(i, st->map_string_id(sid));
                                }
                            }
                            ...
                            if (dense->has_denseinfo()) {
                                ...
                                for (int i=0, l= denseinfo->user_sid_size(); i<l; i++) {
                                    // map interim string-ids > 0 to real string ids
                                    uint16_t user_sid = st->map_string_id(denseinfo->user_sid(i));
                    ...
                    if (pbf_relations) {
                        ...
                        for (int i=0, l=pbf_relations->relations_size(); i<l; i++) {
                            ...
                            for (int mi=0, ml=relation->roles_sid_size(); mi<ml; mi++) {
                                relation->set_roles_sid(mi, st->map_string_id(relation->roles_sid(mi)));
                            }
                        }
                    }
                }
                ...
                template <class pbf_object_t> void map_common_string_ids(pbf_object_t *in) {
                    ...
                    if (in->has_info()) {
                        ...
                        info->set_user_sid(st->map_string_id(info->user_sid()));
                    }

                    // iterate over all tags and map the interim-ids of the key and the value to real ids
                    for (int i=0, l=in->keys_size(); i<l; i++) {
                        in->set_keys(i, st->map_string_id(in->keys(i)));
                        in->set_vals(i, st->map_string_id(in->vals(i)));
                    }
                }
                ...
                template <class pbf_object_t> void apply_common_info(Osmium::OSM::Object *in, pbf_object_t *out) {
                    ...
                    for (int i=0, l=in->tag_count(); i<l; i++) {
                        out->add_keys(st->record_string(in->get_tag_key(i)));
                        out->add_vals(st->record_string(in->get_tag_value(i)));
                    }
                    ...
                    if (!omit_metadata()) {
                        ...
                        out_info->set_user_sid(st->record_string(in->get_user()));
                    }
                }
                ...
                void store_primitive_block() {
                    ...
                    st->store_stringtable(pbf_primitive_block.mutable_stringtable());
                ...
                void write_dense(Osmium::OSM::Node *node) {
                    ...
                    for (int i=0, l=node->tag_count(); i<l; i++) {
                        dense->add_keys_vals(st->record_string(node->get_tag_key(i)));
                        dense->add_keys_vals(st->record_string(node->get_tag_value(i)));
                    }
                    ...
                    if (!omit_metadata()) {
                        ...
                        denseinfo->add_user_sid(st->record_string(node->get_user()));
                    }
                }
                ...
                void write_classic(Osmium::OSM::Relation *relation) {
                    ...
                    for (int i=0, l=relation->member_count(); i<l; i++) {
                        ...
                        // interim string-id to the pbf-object
                        pbf_relation->add_roles_sid(st->record_string(mem->get_role()));
            ...
            public:
                ...
                PBF() : Base(),
                    ...
                    // create StringTable management object
                    st = new Osmium::StringTable();
                }
